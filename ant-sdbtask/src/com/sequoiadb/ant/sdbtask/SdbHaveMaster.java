...
public class SdbHaveMaster extends Task {
	...
	private boolean checkMaster(ReplicaGroup RG, Sequoiadb sdb) {
		...
		try {
			...
			BasicBSONList bson_list = (BasicBSONList) RG.getDetail().get(
					"Group");
			...
			for (int i = 0; i < nodeNum; i++) {
				try {
					...
					if (sdb.getSnapshot(7,
							"{GroupID:" + groupID + ",NodeID:" + nodeID + "}",
							"{\"IsPrimary\":null}", null).hasNext()) {
						String isMaster = sdb
								.getSnapshot(
										7,
										"{GroupID:" + groupID + ",NodeID:"
												+ nodeID + "}",
										"{\"IsPrimary\":null}", null).getNext()
